...
def parse_train_list(ann_dir):
    ...
    img_list = "/ds-av/public_datasets/imagenet/raw/ImageSets/VID/train.txt"
    ...
    print(f"LENGHT OF MODIFIED TRAIN 2015 : {len(train_infos)}")
    return train_infos


def parse_val_list(ann_dir):
    """Parse the txt file of ImageNet VID val dataset."""
    img_list = "/netscratch/conversion_scripts/image_net_vid_2015_val.txt"
    img_list = mmcv.list_from_file(img_list)
    print(f"length of list : {len(img_list)}")


    val_infos = defaultdict(list)
    for info in img_list:
        if "ILSVRC2017_val" in info:
            continue
        info = info.split(' ')
        val_infos[info[0]] = dict(num_frames=int(info[-1]))

    print(f"length of list : {len(val_infos)}")
    return val_infos
