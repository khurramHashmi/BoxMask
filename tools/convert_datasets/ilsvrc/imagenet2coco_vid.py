...
def parse_train_list(ann_dir):
    ...
    img_list = "/ds-av/public_datasets/imagenet/raw/ImageSets/VID/train_9.txt"
    img_list = mmcv.list_from_file(img_list)
    train_infos = defaultdict(list)
    for info in img_list:
        info = info.split(' ')
        '''
            MODIFICATION TO PARSE TRAIN FILES
            Since, info[2] does not exist in the train.txt file, we shall use info[1] fir video_train_frames.
        '''
        if info[0] not in train_infos:
            train_infos[info[0]] = dict(vid_train_frames=[int(info[1]) - 1], num_frames=int(info[-1]))
        else:
            train_infos[info[0]]['vid_train_frames'].append(int(info[1]) - 1)
    return train_infos


def parse_val_list(ann_dir):
    """Parse the txt file of ImageNet VID val dataset."""
    img_list = "/ds-av/public_datasets/imagenet/raw/ImageSets/VID/val.txt"
    img_list = mmcv.list_from_file(img_list)

    # CHANGE HERE : limiting Val to smaller size such that experiments can be performed

    img_list = img_list[:500]

    print(f"LENGTH OF VAL IMG LIST : {len(img_list)}")
...
def convert_vid(VID, ann_dir, save_dir, mode='train'):
    ...
    xml_dir = osp.join(ann_dir, 'Annotations/VID/{}'.format(mode))
    ...
    for vid_info in tqdm(vid_infos):
        ...
        for frame_id in range(num_frames):
            ...
            #CHANGE HERE SINCE XML FILES ARE NOT IN NESTED DIRECTORIES
    ...
    mmcv.dump(VID, osp.join(save_dir, f'imagenet_vid_{mode}_500.json'))
...
def main():
    ...
    # FOR NOW BOTH TRAIN AND VAL IS GENERATED ON TRAIN9.TXT FILE
