...
def convert_det(DET, ann_dir, save_dir):
    ...
    vid_train_img_list = "/ds-av/public_datasets/imagenet/raw/ImageSets/DET/train.txt"
    vid_train_img_list = mmcv.list_from_file(vid_train_img_list)
    vid_train_img_names = []
    for vid_train_img_info in vid_train_img_list:
        if "extra" in vid_train_img_info:
            continue
        else:
    ...
    for img_name in tqdm(vid_train_img_names):
        ...
        xml_path = osp.join(ann_dir, 'Annotations/DET/train',
                            img_name.replace('JPEG', 'xml'))
        records, DET, obj_num_classes = parse_xml(img_name, xml_path, False,
                                                  records, DET,
                                                  obj_num_classes)

    for sub_set in tqdm(dataset_sets):

        sub_set_base_path = osp.join(ann_dir, 'Annotations/DET/train', sub_set)
    ...
    mmcv.dump(DET, osp.join(save_dir, 'imagenet_det_30plus1cls_val.json'))
    print('-----ImageNet DET------')
