...
class MaskAnchorHead(BaseDenseHead, BBoxTestMixin, StandardRoIHead):
    ...
    def __init__(self,
        ...
        super(MaskAnchorHead, self).__init__(init_cfg)
    ...
    def forward_train(self,
        ...
        losses = self.loss(*loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore,input_features=x)
    ...
    def _get_targets_single(self,
        ...
        # print("OUTPUT VALUES THAT WILL BE RETURNED FROM GET TARGETS SINGLE")
        # print(f" sampling_result {sampling_result} mask_targets {mask_targets} mask_pred {mask_pred} ")
    ...
    def get_targets(self,
        ...
        multiple images.
        Also compute mask targets for anchors in multiple images.
        ...
        Args:
            ...
            gt_masks_list (list[BitMmapMasks) Ground truth masks for each bboxes.
        ...
        Returns:
            tuple: Usually returns a tuple containing learning targets.
                ...
                - bbox_targets_list (list[Tensor]): BBox targets of each level.
                - bbox_weights_list (list[Tensor]): BBox weights of each level.
                - mask_targets_list (list[Tensor]): Mask targets of each level.
                - mask_weights_list (list[Tensor]): Mask weights of each level.
        ...
        '''Code to create mask target starts here'''
        # mask_pos_assigned_gt_inds = torch.zeros_like(sampling_results_list.pos_assigned_gt_inds)


        pos_proposals = [res.pos_bboxes for res in sampling_results_list]
        pos_assigned_gt_inds = [
            res.pos_assigned_gt_inds for res in sampling_results_list
        ]

        # print(f"pos_proposals length: {len(sampling_results_list.pos_bboxes)} and pos assigned gt ind : {len(sampling_results_list.pos_assigned_gt_inds)}")
        mask_targets = mask_target(pos_proposals, pos_assigned_gt_inds, gt_masks_list, self.train_cfg)
        # print(f"Returned mask targets : {mask_targets.shape}")

        '''Code to create mask target ENDS here'''
        ...
        bbox_weights_list = images_to_levels(all_bbox_weights,
                                             num_level_anchors)
        # mask_targets = images_to_levels(mask_targets, num_level_anchors)


        res = (labels_list, label_weights_list, bbox_targets_list,
               bbox_weights_list, num_total_pos, num_total_neg, mask_targets)
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
                    bbox_targets, bbox_weights, mask_preds, mask_targets, num_total_samples):
        ...
        print(f"mask_targets_list {mask_targets[0].shape} mask_preds {mask_preds[0].shape} and DIMENSION LENGTH : {mask_preds[0].ndim}")
        ...
        print(f"Mask Prediction :·{len(mask_preds)} Mask Prediction shape :·{mask_preds.shape} and mask_targets shape {mask_targets.shape} and lenght : {len(mask_targets)}and bbox weights {bbox_weights.shape}")
        # loss_mask = self.mask_head.loss(mask_results['mask_pred'], mask_targets, pos_labels)
        loss_mask=self.mask_head.loss(mask_preds[0], mask_targets, bbox_weights)
    ...
    def loss(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos, num_total_neg, mask_targets_list, mask_preds) = cls_reg_targets
        num_total_samples = (
        ...
        losses_cls, losses_bbox, losses_mask = multi_apply(
            ...
            mask_targets_list=mask_targets_list)
        return dict(loss_cls=losses_cls, loss_bbox=losses_bbox, loss_mask=losses_mask)
