...
'''
    change was done in this script to parse the output when adding mask in the script.
    Orignal script is avaialable at github
    Change done on line 297-300
    Change is the replace out[0] to outs[0][0] which contains bbox results while outs[0][1] contains mask results
    ONLY HAPPENS WHEN MASK IS INCLUDED IN THE NETWORK
'''
    ...
    def simple_test(self,
        ...
        if hasattr(self.detector, 'roi_head'):
            if proposals is None:
                ...
                proposal_list = proposals
            #CHANGE HERE, ADDING DEFAULT ORI_SHAPE IN META SUCH THAT DEMO SCRIPT CAN WORK
            # iTS A HACK AND IT SHOULD BE REMOVED IF CAUSING ANY PROBLEM
            #IT ONLY WORKS HERE WITH DIMENSION OF THE CORRESPONDING VIDEO FRAMES
            for index in range(len(img_metas)):
                img_metas[index]["ori_shape"] = (720, 1280, 3)
            ## HACK CODE ADDING ORI SHAPE ENDS HERE
        ...
        results = dict()
        results['det_bboxes'] = outs[0][0]

        #COMMENTING THIS IF NOT TO PROPAGATE MASK RESULTS DURING TESTING
        if len(outs[0]) == 2:
            results['det_masks'] = outs[0][1]
