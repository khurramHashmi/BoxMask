...
class TemporalRoIAlign(SingleRoIExtractor):
    ...
    def forward(self, feats, rois, roi_scale_factor=None, ref_feats=None):
        """Forward function."""
        # print(f"Default FEATURES: {len(feats)} AND ONE FEAT SHAPE : {feats[0].shape}")
        #
        # print(f"Default ROI shape : {rois.shape}")
        # print(f"Default roi_scale_factor: {roi_scale_factor}")
        ...
        if ref_feats is None:
            ...
            # print(f"Length of Ref Features : {len(ref_feats)}")
            # print(f"OUTPUT ROI featues shape : {roi_feats.shape} and REF shape : {ref_feats[-1].shape}")
            # print(f"OUTPUT REF ROI featues shape : {ref_roi_feats.shape} ")
            # import sys
            # sys.exit()
